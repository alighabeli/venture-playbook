...
# Try to import pyyaml and markdown; if missing, print instructions
try:
    import yaml
except ImportError:
...
try:
    ...
    print("[ERROR] Python-Markdown is required. Run: pip install markdown")
    exit(1)
...
def parse_vos_file(file_path: str) -> Tuple[Optional[Dict], str]:
    ...
    try:
        ...
        if not match:
            ...
            return None, markdown.markdown(content, extensions=['tables', 'fenced_code', 'codehilite'])
            
        yaml_data = yaml.safe_load(match.group(1))
        markdown_data = match.group(2)
        html_body = markdown.markdown(markdown_data, extensions=['tables', 'fenced_code', 'codehilite'])
